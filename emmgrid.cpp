...
int main()
{
	...
	MAGtype_CoordSpherical CoordSpherical;//球形坐标
	MAGtype_Geoid Geoid;//大地水准面
	...
	for (Epoch = 0; Epoch < epochs; Epoch++)
	{
		...
		if (Epoch == epochs - 1)
			Epoch++;
		if (!MAG_robustReadMagneticModel_Large(filename, filenameSV, &MagneticModels[Epoch])) {//读取EMD%D.COF文件
